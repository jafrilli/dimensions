...
module.exports.run = async (msg, client, args) => {
    ...
    switch (args[0]) {
        ...
        case "role": 
            ...
            break;
        case "toggle": 
            await modToggle(msg, client, args, officerDimension, officerRole);
            removedID();
            break;
        case "help":
            ...
            break;
        case "rr": 
            await modRR(msg, client, args, officerDimension, officerRole);
            removedID();
            break;
        case "dimension":
...
// toggle series
async function modToggle(msg, client, args, officerDimension) {
    if(args[1]) {
        switch(args[1]) {
            case "welcome":
                await toggleFunctions.toggleWelcome(msg, client, args, officerDimension);
                break;
            default:
                await msg.channel.send(`That was an invalid '>mod toggle' argument. Use \'>mod toggle help\' to see different commands regarding roles`)
                return;
        }
    } else {
        msg.channel.send(`Type \'>mod toggle help\' for everything related to roles!`);
    }
}
const toggleFunctions = {
    toggleWelcome: async (msg, client, args, officerDimension) => {
        await df.dimensionUpdate(
            client,
            officerDimension,
            "welcome",
...
// rr series
async function modRR(msg, client, args, officerDimension, officerRole) {
    if(args[1]) {
        switch(args[1]) {
            case "create":
                await rrFunctions.modRRCreate(msg, client, args, officerDimension);
                break;
            case "edit":
                await rrFunctions.modRREdit(msg, client, args, officerDimension, officerRole);
                break;
            case "help":
                await rrFunctions.modRRHelp(msg, client, args, officerDimension);
                break;
            default:
                await msg.channel.send(`That was an invalid '>mod rr' argument. Use \'>mod rr help\' to see different commands regarding roles`)
                return;
        }
    } else {
        msg.channel.send(`Type \'>mod role help\' for everything related to roles!`);
...
const rrFunctions = {
    modRRCreate: async (msg, client, args, officerDimension, officerRole) => {
    ...
    modRREdit: async (msg, client, args, officerDimension) => {
        const quit = () => msg.channel.send('Quit RREdit wizard.');
    
        var channelAndMessageID = await wizard.default(
            msg, client, false, null, 
            {title: "Edit Reaction Role Message | Message Link"},
            (message) => {
                var array = message.content.split('/');
                if(array.length < 3) {
                    msg.channel.send("That is not a valid link!")
                } else if(array[array.length-1].length < 14) {
                    msg.channel.send("There is a problem with your link. Make sure there isn't a \'/\' at the end of the link");
                } else if(array[array.length-1].length > 14 && array[array.length-2].length > 14) {
                    return [array[array.length-2], array[array.length-1]];
                }
            },
            {title: "Edit Reaction Role Message | Message Link"}
        );
        if(channelAndMessageID === false) return quit();
        
        // title
        var title = await wizard.type.text(
            msg, client, true, null, {title: "Edit Reaction Roles: Title", description: "Title of the embed:"}
        );
        if (title === false) return quit();
    
        // description
        var description = await wizard.type.text(
            msg, client, true, null, {title: "Edit Reaction Roles: Description", description: "Description of the embed:"}
        );
        if (description === false) return quit();
    
        // color
        var color = await wizard.type.color(
            msg, client, true, null, {title: "Edit Reaction Roles: Color", description: "(skippable) Enter the hex code of the color of the embed (hashtag included) like \'#FFFFFF\'."},
            {title: "Reaction Roles: Color", description: "WRONG INPUT (skippable) Enter the hex code of the color of the embed (hashtag included) like \'#FFFFFF\'."}
        )
        if (color === false) return quit();
    
        // graphic
        var graphic = await wizard.type.graphic(
            msg, client, true, null, {title: "Edit Reaction Roles: Graphic", description: "Link of the new graphic of the embed:"}
        );
        if (graphic === false) return quit();
    
        // list roles under description
        var shouldList = await wizard.type.yesno(
            msg, client, true, 'no', {title: "Edit Reaction Roles: Role List", description: "Would you like to list the roles and their emojis under your description? Respond \'yes\' or \'no\'!"},
            {title: "Add Role List?", description: "Invalid Response! You must respond with either \'yes\' or \'no\' on whether you would like to list the roles and their emojis under your description."}
        );
        if (shouldList === false) return quit();
        shouldList = shouldList.toLowerCase() == 'yes' ? true : false;
    
        var rrmsg = client.cache.rrmessages.get(channelAndMessageID[1]).reactionRoles;
        console.log(rrmsg);
        // generate role list
        var roleList = "\n";
        shouldList ? Object.keys(rrmsg).forEach(emojiId => {
            if(msg.guild.emojis.get(emojiId)) {
                var emoji = msg.guild.emojis.get(emojiId)
                var str = "<";
                emoji.animated ? str += 'a' : '';
                roleList += `${str}:${emoji.name}:${emojiId}> | <@&${rrmsg[emojiId]}>\n`;
            } else {
                roleList += `${emojiId} | <@&${rrmsg[emojiId]}>\n`;
            }
        }) : null;
        ...
        rrEmbed.setDescription(shouldList ? (description ? description + roleList : roleList) : (description ? description : ""));
        rrEmbed.setColor(color);
        rrEmbed.setImage(graphic)
        // var rrEmbMsg = await msg.guild.channels.get(channel.id).send(rrEmbed);
        try {
            // ! THIS IS NOT THE CORRECT WAY TO FETCH THE MESSAGE YOU WANT TO EDIT. ASK DISCORD.JS DISCORD SERVER
            var message = await msg.guild.channels.get(channelAndMessageID[0]).messages.fetch(channelAndMessageID[1]);
            await message.edit(rrEmbed);
        } catch (err) {
            return functions.embed.errors.catch(err, client);
        }
        return msg.channel.send('Successfully edited the RR!');
    },
    modRRHelp: async (msg, client, args, officerDimension) => {
        const embed = new MessageEmbed({
            title: "__**Mod RR Commands Help**__",
            description: "All of these commands __do not__ need arguments (text after them). They are all setup wizards. Type \'quit\' at anytime during the setup wizard to cancel the process.",
            fields: [
                {name: ">mod rr create", value: `Setup wizard for a new role for <@&${officerDimension}>.`},
                {name: ">mod rr edit", value: `Setup wizard to delete a role from <@&${officerDimension}>.`},
                {name: "HOW TO DELETE RR", value: "Just delete it like you would a normal message!"},
                {name: ">mod rr help", value: `Lists all available commands (this)`},
            ]
        });
